...
## Requirements
- **NEW**: The temporary copy of the reference file MUST be deleted at the end of the `ScanReferenceFile` routine, regardless of success or failure.
- **NEW**: The deletion operation MUST fail gracefully (e.g., using `On Error Resume Next`) if the file cannot be deleted, preventing the tool from throwing an error over trivial cleanup failures.
